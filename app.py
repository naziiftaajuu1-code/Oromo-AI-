...
def refresh_telegram_status(expected_url: str = "") -> bool:
    """Ask Telegram for the real webhook state instead of trusting local state."""
    try:
        ...
        api_ok = True
        webhook_ok = bool(current_url) and (not expected_url or current_url == expected_url)

        with state_lock:
            state["telegram_api_ok"] = api_ok
            state["webhook_configured"] = webhook_ok
            state["telegram_webhook_url"] = current_url
            state["telegram_pending_updates"] = pending
            state["telegram_last_error"] = last_error

        logger.info(
            "[TELEGRAM] API=OK webhook=%s pending=%s error=%s",
            "OK" if webhook_ok else "MISSING/WRONG",
            pending,
            last_error or "none",
        )
        return webhook_ok
...
def configure_webhook() -> bool:
    ...
    for attempt in range(5):
        try:
            # Validate the bot token before changing webhook state.
            me = bot.get_me()
            if not getattr(me, "id", None):
                raise RuntimeError("Telegram get_me returned no bot id")
            ...
            # Confirm with Telegram that the webhook really exists.
            if refresh_telegram_status(expected_url=url):
                logger.info("[WEBHOOK] configured and verified: %s", url)
                return True
...
def webhook_monitor_loop() -> None:
    """Keep the webhook correct if Telegram or a deploy changes it later."""
    while True:
        time.sleep(120)
        try:
            expected = public_base_url() + WEBHOOK_PATH if public_base_url() else ""
            if not expected or not refresh_telegram_status(expected_url=expected):
                logger.warning("[WEBHOOK] verification failed; attempting reconfiguration")
                configure_webhook()
        except Exception:
            logger.exception("[WEBHOOK] monitor failed")
...
@app.get("/health")
def health():
    # Health must stay 200 for Render even while Telegram is temporarily unavailable.
    # Use /status for detailed dependency diagnostics.
    with state_lock:
        webhook_ok = bool(state["webhook_configured"])
        telegram_ok = bool(state["telegram_api_ok"])
    return jsonify(
        {
            "status": "ok",
            "telegram_api_ok": telegram_ok,
...
@app.get("/status")
def status():
    # Refresh Telegram's real webhook state on demand. This makes diagnosis possible
    # without exposing the bot token. It does not call the Hugging Face model.
    expected = public_base_url() + WEBHOOK_PATH if public_base_url() else ""
    refresh_telegram_status(expected_url=expected)
    ...
    payload["hf_provider"] = HF_PROVIDER
    payload["telegram_webhook_path"] = WEBHOOK_PATH
...
def telegram_webhook():
    ...
    try:
        ...
        logger.info("[WEBHOOK] update received: %s", update_id)

        if is_duplicate_update(update_id):
            return jsonify({"ok": True, "duplicate": True}), 200

        # Return HTTP 200 quickly; heavy work runs outside Flask's request thread.
        future = UPDATE_POOL.submit(bot.process_new_updates, [update])

        def _log_update_worker_result(f):
            exc = f.exception()
            if exc is not None:
                logger.error("[WEBHOOK] update worker failed: %s", exc, exc_info=exc)

        future.add_done_callback(_log_update_worker_result)
